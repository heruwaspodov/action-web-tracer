...
import { approvedCdpDomains, CdpAdapter, type DebuggerTransport } from '../src/background/cdp-adapter.js';
...
describe('CDP target orchestration', () => {
	it('enables only approved domains and normalizes same-process, main, and OOPIF identities', async () => {
		...
		expect(transport.commands.map((command) => command.method)).toEqual(['Network.enable', 'Runtime.enable', 'Log.enable', 'Page.enable', 'Target.setAutoAttach']);
		...
		expect(transport.commands.filter((command) => command.sessionId === 'child').map((command) => command.method)).toEqual(approvedCdpDomains.filter((domain) => domain !== 'Target').map((domain) => `${domain}.enable`));
	});
